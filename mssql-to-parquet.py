...
SELECT 
    ...
    OBJECT_NAME(referencing_id) referencing_obj_name,
    * 
FROM 
    sys.sql_expression_dependencies
"""

view_col_details_sql = """
...
if __name__ == "__main__":
    hostname='bidatacentre'
    port='1433'
    engine = create_engine(f"mssql+pymssql://{hostname}:{port}")
    with engine.connect() as connection:
        databases = pd.read_sql("select name from sys.databases", con=connection)
        databases.to_parquet('data/databases.parquet', compression="snappy")

    for db in databases['name']:
        engine = create_engine(f"mssql+pymssql://{hostname}:{port}/{db}")
        with engine.connect() as connection:
            tables = pd.read_sql("select * from sys.tables", con=connection)
            tables.to_parquet(f"data/{db}.tables.parquet", compression="snappy")
            views = pd.read_sql("select * from sys.tables", con=connection)
            views.to_parquet(f"data/{db}.views.parquet", compression="snappy")
            refs = pd.read_sql(refs_sql, con=connection)
            refs.to_parquet(f"data/{db}.refs.parquet", compression="snappy")
            table_col_details = pd.read_sql(table_col_details_sql, con=connection)
            table_col_details.to_parquet(f"data/{db}.table-col-details.parquet", compression="snappy")
            view_col_details = pd.read_sql(view_col_details_sql, con=connection)
            view_col_details.to_parquet(f"data/{db}.view-col-details.parquet", compression="snappy")
